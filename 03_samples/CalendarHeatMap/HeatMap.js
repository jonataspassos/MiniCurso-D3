...
class CalendarHeatMap {
    constructor(chartConfig) {
        this.validData().create().draw().addInteractions();
    }
    validData() {
        var a = this;
        this.chartConfig = chartConfig;

        if (a.chartConfig.chart.calendar && a.chartConfig.chart.hour) {//No chart solicited
            console.error("No chart Soloicited. Review chart config.");
            return;
        }
        ...
        if (a.chartConfig.layout.margin.bottom == undefined) a.chartConfig.layout.margin.bottom = 20;
        if (a.chartConfig.layout.margin.left == undefined) a.chartConfig.layout.margin.left = 20;
    ...
    create() {//in this functions will be created all the components used in this chart.
        ...
        this.svg = a.chartConfig.svg ?
            ...
            d3.select(a.chartConfig.parent).append("svg").attr("id", chartConfig.name + "-container");
    ...
    draw() {
        ...
        if (!a.chartConfig.chart.calendar) {
            //re-size text based in width of text week's axis
            var temp = String.adjustWidth("00/XXX", a.chartConfig.layout.font_size, a.width * 0.15);
            this.font_size = Math.min(this.chartConfig.layout.font_size, temp);
        }
        if (!a.chartConfig.chart.hour) {
            //re-size text based in width of text hour's axis
            var temp = String.adjustWidth(a.chartConfig.hour.model == 4 ? "00h-00h" : "00XX", a.chartConfig.layout.font_size, a.width * 0.15);
            this.font_size = Math.min(this.chartConfig.layout.font_size, temp);
        }
        ...
        if (!a.chartConfig.chart.hour) {
            ...
            this.hour.totalRects.select("rect")
                ...
                .attr("fill", function (d) {
                    return a.chartConfig.layout.colors(color(d.value));
                });
        ...
        d3.select("body").on("keydown", function (event) {
            if (a.calendar.on) {
                if (d3.event.keyCode == 38) {//up Arrow
                    ...
                    a.scrolMove(a.calendar.scrolposition - 1, a.calendar, a.chartConfig.calendar);
                } else if (d3.event.keyCode == 40) {//down Arrow
                    d3.event.preventDefault();
                    a.scrolMove(a.calendar.scrolposition + 1, a.calendar, a.chartConfig.calendar);
                } else if ((d3.event.keyCode == 34 || d3.event.keyCode == 32)) {//page Down || space
                    d3.event.preventDefault();
                    a.scrolMove(a.calendar.scrolposition + a.chartConfig.calendar.extrapolation, a.calendar, a.chartConfig.calendar);
                    ...
                    d3.event.preventDefault();
                    a.scrolMove(a.calendar.scrolMax, a.calendar, a.chartConfig.calendar);
                } else if (d3.event.keyCode == 36) {//home
                    d3.event.preventDefault();
                    a.scrolMove(0, a.calendar, a.chartConfig.calendar);
                }
            }
            if (a.hour.on) {
                if (d3.event.keyCode == 38) {//up Arrow
                    ...
                    d3.event.preventDefault();
                    a.scrolMove(a.hour.scrolposition + a.chartConfig.hour.extrapolation, a.hour, a.chartConfig.hour);
                } else if (d3.event.keyCode == 33) {//page up
                    d3.event.preventDefault();
                    a.scrolMove(a.hour.scrolposition - a.chartConfig.hour.extrapolation, a.hour, a.chartConfig.hour);
                    ...
                    d3.event.preventDefault();
                    a.scrolMove(a.hour.scrolMax, a.hour, a.chartConfig.hour);
                } else if (d3.event.keyCode == 36) {//home
                    d3.event.preventDefault();
                    a.scrolMove(0, a.hour, a.chartConfig.hour);
                }
            }
    ...
    drawChart(chart, chartConfig) {
        ...
        chart.rects.attr("fill", function (d) { return (d.value - min) / den > 0.8 ? "#fff" : "#000"; }).select("rect")
            ...
            .attr("fill", function (d) {
                ...
                return chartConfig.colors(color(d.value));
            });
    ...
    scrolEvents(chart, chartConfig) {
        ...
        var build = function (d) {
            if (d3.event.sourceEvent == undefined || d3.event.sourceEvent.deltaY == undefined)
                return;
            var param = d3.event.sourceEvent.deltaY / 125;
    ...
    scrolMove(position, chart, chartConfig) {
        ...
        if (chart.extrapolation) {//Do only if exists extrapolation
            if (chart.scrolposition == position) {
                return;//Not Do if already in position
            }
    ...
    dataSetter() {//Configurations of dataset
        ...
        if (!a.chartConfig.chart.hour) {
            ...
            for (var i = 0; i < 7 * model; i++) {
                ...
                if (model == 4) this.hour.data[i].hour = (i % 4) * 6;
    ...
    titleConstruct() {//construct Title
        var a = this;
        var b = a.chartConfig.dataConfig;
