...
withBubbleDataDo((dataset) => {
    ...
    /*global d3*/
    let svg = tag.append("svg").attr("width",size.w).attr("height",size.h)
    let width = size.w; // get width in pixels
    let height = size.h;
    ...
    let nodes = pack(root).leaves().map(node => {
        ...
        return {
            ...
            id: 'dp.' + data.ideCadastro,
            ideCadastro: data.ideCadastro,
            cat: data.cat,
            name: data.nomeParlamentar,
            value: data.value,
            icon: data.urlFoto,
            data: "Deputad"+(data.sexo=="masculino"?"o":(data.sexo=="feminino"?"a":"o(a)"))+" Federal<br/>"+
                    "<strong>Nome:</strong> "+data.nomeParlamentar+"<br/>"+
                    "<strong>Sexo:</strong> "+data.sexo+"<br/>"+
                    (data.PP?"<strong>Partido:</strong> "+data.PP:"")+" <strong>Estado:</strong> "+data.uf+"<br/>"+
                    "<strong>Numero de Projetos de Lei:</strong> "+data.value,
            desc:"Deputad"+(data.sexo=="masculino"?"o":(data.sexo=="feminino"?"a":"o(a)"))+" Federal "+
                    data.nomeParlamentar
        };
    });
    ...
    */
   //O uso do less garante que o circulo será proporcional ao svg
    let infoBox = node.append('foreignObject')
        .classed('circle-overlay hidden', true)
        .attr('x', -less * 0.5 * 0.4)
        .attr('y', -less * 0.5 * 0.4)
        .attr('height', less * 0.4)
        .attr('width', less * 0.4)
        .append('xhtml:div')
